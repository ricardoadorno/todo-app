...
import { mockTransactions, mockInvestments } from "@/lib/mock-data";
...
export default function FinancesPage() {
  ...
  useEffect(() => {
    ...
    setInvestments(mockInvestments.sort((a,b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()));
  }, []);
  ...
  const handleAddTransaction = (data: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    toast({ title: "Transação Adicionada", description: `"${newTransaction.description}" foi adicionada com sucesso.` });
  };
  
  const handleEditTransaction = (data: Omit<Transaction, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (!editingTransaction) return;
    const updatedTransaction: Transaction = { ...editingTransaction, ...data, updatedAt: new Date().toISOString() };
    setTransactions(prev => prev.map(t => t.id === editingTransaction.id ? updatedTransaction : t).sort((a,b) => new Date(b.date).getTime() - new Date(a.date).getTime()));
    setIsTransactionFormOpen(false);
    setEditingTransaction(null);
    toast({ title: "Transação Atualizada", description: `"${updatedTransaction.description}" foi atualizada.` });
  };
  ...
  const handleDeleteTransaction = (transactionId: string) => {
    ...
    setTransactions(prev => prev.filter(t => t.id !== transactionId));
     if (transaction) {
      toast({ title: "Transação Excluída", description: `"${transaction.description}" foi excluída.`, variant: "destructive" });
    }
  };
  ...
  const handleAddInvestment = (data: Omit<Investment, 'id' | 'createdAt' | 'updatedAt'>) => {
    ...
    setInvestments(prev => [newInvestment, ...prev].sort((a,b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()));
    setIsInvestmentFormOpen(false);
    setEditingInvestment(null);
    toast({ title: "Investimento Adicionado", description: `"${newInvestment.name}" foi adicionado.` });
  };

  const handleEditInvestment = (data: Omit<Investment, 'id' | 'createdAt' | 'updatedAt'>) => {
    if (!editingInvestment) return;
    const updatedInvestment: Investment = { ...editingInvestment, ...data, updatedAt: new Date().toISOString() };
    setInvestments(prev => prev.map(inv => inv.id === editingInvestment.id ? updatedInvestment : inv).sort((a,b) => new Date(b.purchaseDate).getTime() - new Date(a.purchaseDate).getTime()));
    setIsInvestmentFormOpen(false);
    setEditingInvestment(null);
    toast({ title: "Investimento Atualizado", description: `"${updatedInvestment.name}" foi atualizado.` });
  };
  ...
  const handleDeleteInvestment = (investmentId: string) => {
    ...
    if (investment) {
      toast({ title: "Investimento Excluído", description: `"${investment.name}" foi excluído.`, variant: "destructive" });
    }
  };
        ...
        <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          ...
          <Card>
           ...
           <Card>
            ...
            <CardContent>
              <div className="text-2xl font-bold text-blue-600">R${totalInvested.toFixed(2)}</div>
            </CardContent>
          </Card>
        </div>
                      ...
                      <TableCell className="text-right space-x-1">
                        ...
                        <Dialog>
                          ...
                          <DialogContent className="sm:max-w-md">
                            ...
                            <p>Tem certeza que deseja excluir a transação "{t.description}" no valor de R${t.amount.toFixed(2)}?</p>
                            <DialogFooter>
